...
Zotero_Preferences.General = {
	init: function () {
		...
		document.getElementById("general-checkbox-automaticSnapshots").onchange = function () {
			Zotero.Prefs.set('automaticSnapshots', this.checked);
		};
		document.getElementById("general-checkbox-downloadAssociatedFiles").onchange = function () {
			Zotero.Prefs.set('downloadAssociatedFiles', this.checked);
		};
...
Zotero_Preferences.Components.Proxies = React.createClass({
	...
	render: function () {
		...
		if (this.state.proxies.length) {
			configuredProxies = this.state.proxies.map((proxy, i) => React.createElement(
				"option",
				{ value: i, key: i, selected: this.state.currentProxyIdx == i },
				proxy.scheme
			));
		} else {
			configuredProxies = React.createElement("option", { value: -1 });
		}
		...
		if (this.props.transparent && this.state.currentProxy) {
			...
			let configuredHosts = currentProxy.hosts.map((host, i) => React.createElement(
				"option",
				{ value: i, key: i, selected: this.state.currentHostIdx == i },
				host
			));

			let disableAddHost = currentProxy.hosts.length && currentProxy.hosts[currentProxy.hosts.length - 1].trim().length == 0;

			proxySettings = React.createElement(
				"div",
				...
				React.createElement(
					...
					React.createElement(
						...
						"\xA0Automatically convert hyphens to dots in proxied hostnames"
					),
					React.createElement("br", null)
				),
				React.createElement(
					...
					React.createElement("input", { style: { flexGrow: "1" }, type: "text", name: "scheme", onChange: this.handleSchemeChange, value: currentProxy.scheme })
				),
				React.createElement(
					"p",
					null,
				...
				React.createElement(
					...
					React.createElement(
						"select",
						{ size: "8", multiple: true, onChange: this.handleHostSelectChange },
						configuredHosts
					),
					React.createElement(
						"p",
						...
						React.createElement("input", { style: { minWidth: "80px", marginRight: "10px" }, type: "button", onClick: this.handleHostButtonClick, value: "-" })
					),
					React.createElement(
						"p",
						...
						React.createElement("input", { style: { flexGrow: '1' }, type: "text", value: currentProxy.hosts[this.state.currentHostIdx] || '', onChange: this.handleHostnameChange, ref: "hostInput" })
					)
				)
			);
		}
		...
		return React.createElement(
			...
			React.createElement(
				"select",
				{ size: "8", multiple: true, onChange: this.handleProxySelectChange, disabled: !this.props.transparent },
				configuredProxies
			),
			React.createElement(
				"p",
...
Zotero_Preferences.Components.MIMETypeHandling = React.createClass({
	...
	render: function () {
		...
		if (this.state.hosts.length) {
			...
			hosts = React.createElement("option", { value: -1 });
		}
		let hostname = '';
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					"select",
					{ size: "8", multiple: true, onChange: this.handleSelectChange },
					hosts
				),
				React.createElement(
					"p",
					null,
					" ",
					React.createElement("input", { style: { minWidth: "80px", marginRight: "10px" }, type: "button", onClick: this.handleHostRemove, disabled: this.state.currentHostIdx == -1, value: "Remove" }),
					" "
				),
				hostname
			)
